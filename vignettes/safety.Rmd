---
title: "The analysis of safety data from clinical trials"
...
statistical technique to compare, say, mean values, or the duration of
survival, or the patients in the different treatment groups.

However, the bulk of the data that are collected in clinical trials relate
not to the _efficacy_ of the drug, but to its _safety_. Safety
...
be known until the data have been studied in some detail, and the data
...
Published statistical guidance for clinical trials [-@e9] summarizes this
position:
...
* The USA Food and Drug Administration (FDA) has published a
...
*  Rhabdomyolysis, an adverse event that has been linked to certain drugs
...
* In the USA, the National Insitutes of Health and the National Cancer
Institute jointly publish the Common Toxicity Criteria
(CTC) [-@CTC], a guideline listing many laboratory variables and
...
* Various guidelines exist for the diagnosis of kidney failure,
...
_upper limit of normal_ (ULN) and _lower limit of normal_
...
with characterizing the _expected_ response patients have to the drug.
When it comes to safety, it is characterization of the _unexpected_
...
toxicity. An FDA review [-@FDAreview] of the available data states:
...
means and medians went _down_, intuitively suggesting a beneficial
...
Gumbel [-@gumbel] traces attempts by statisticians to handle
extreme value problems back to the early $18^{th}$ century,  but
it was in 1928 that Fisher and Tippett [-@fisherTippett]
...
1958 [-@gumbel]. Many of the statistical applications Gumbel faced were hydrological
...
of the data that are of interest. Beirlant et al [-@beirlant] cite
...
_generalized Pareto_ (GP) distribution.

The GP distribution has two parameters, one of which ($\sigma$) controls the scale of
the distribution, the other of which ($\xi$) controls how heavy the tail is.
By examining how the values of $\sigma$ and $\xi$
...
$\frac{1}{2}$ are shown in the following figure.

```{r sgpd, echo=FALSE, fig.cap="The generalized Pareto distribution with $\\sigma = 1$ and $\\xi$ being $-\\frac{1}{4}, 0$ and $\\frac{1}{2}$. Small values of $\\xi$ result in short-tailed distributions and large values of $\\xi$ result in heavy tails and thus greater probability of outliers."}
x <- seq(0, 7, len=200)
...
```{r shiftplots, echo=FALSE, fig.cap="Plots of pre-treatment values versus on-treatment values in each dose group. The reference lines are lines of identity -- a point above the line indicates an increase from baseline, and point below the line indicates a decrease."}
...
values. As such, it makes sense to eliminate the effect of baseline and
apply extreme value modelling to any remaining variation. Since the
outliers in the shiftplots above are not compatible with the
...
removed) are displayed by Dose group in the following boxplots.

```{r boxplots, echo=FALSE, fig.cap="Boxplots of residuals from using robust regression to eliminate the baseline effect. If the residuals were from a normal distribution, we would expect them to mostly fall between about -2.5 and 2.5 -- within the red dashed lines. Clearly there are outliers indicating that the residuals come from a heavy-tailed distribution. A dose-response relationship now becomes more clear."}
...
the boxplots, it was found that the model that allowed the
shape parameter to depend on log _Dose_ was the best fit to the data.
...
To address this question, we compute _return levels_. The
$M$-patient return level is defined as the level of ALT likely to be
exceeded only once every _M_ patients observed. The figure below
...
```{r rl1000, echo=FALSE, fig.cap="Estimated 1000-patient return levels for ALT at each dose. The heavy line segments are 50\\% confidence intervals, and the lighter segments are 90\\% intervals."}
...
In the figure above, the dose response effect is clearly visible
...
thresholds. Published guidance [-@CTC]  provides the toxicity
grading displayed in the following table, so we use our extreme
...
```{r pexceed, echo=FALSE, fig.cap="Estimated probabilities of ALT exceeding multiples of ULN with 50\\% and 90\\% confidence intervals. Note that the horizontal axes vary from plot to plot."}
...
In the above figure, we can again clearly see the dose-response
...
we are now embarking on a period during which _sound clinical judgement_
