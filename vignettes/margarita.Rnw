...
probabilitiessimpler, as well as functions to support a standard approach to
...
the observed values versus teh fitted values and should show (as it does) a near linear
...
between, with there being few outliers in the lower dose groups and more, and larger,
...
clincial trials, we typically have fewer observations than in other settings, so
...
The function \code{egp3Thresh} calls on \code{egp3RangeFit} from the texmex
package. It then interpolates through the values of $\hatkappa$ and its confidence
...
The function \code{AICtable} provides a neat way of producing \LaTeX output
containin the AICs, log-likelihoods and related information in a written report
...
<<bmod, echo=TRUE, fig.cap="Diagnostic plots for the chosen model.">>=
ggplot(gpd2)
