...
probabilities simpler, as well as functions to support a standard approach to
...
current case, we are interested only in ALT, so we subset down on that and find
the baseline values and post-baseline maxima.

<<subset, echo=FALSE>>=
alt <- getAggregateData(lb, test="lbtestcd", test.value="ALT")
@
...
maximum post-baseline values, though this can be changed.
...
cases, these might be a concatenation of two columns, the `paramcd' column, and
...
shiftplot(alt, aes(base, aval), by=~armcd,
...
mm <- lmr(log(aval) ~ log(base) + ndose, data=alt)
ggplot(mm)
@
...
with there being few outliers in the lower dose groups and more, and larger,
...
Before fitting GPD models, we need to select a threshold above which GPD models
...
parameters in the generalized Pareto distribution above increasing thresholds.
Following Coles~\cite{coles}, these should be constant above a threshold suitable
for thresholding and, to make use of as much data as possible, we want to choose
the lowest threshold possible. Accounting for the uncertainty represented by the
...
is a threshold above which GPD modelling accounting for dose is appropriate.
...
fit GPD models to the residual ALT values by maximum likelihood and look
...
GPD models are likely to fit well above the chosen threshold. The pointwise
simulated envelopes give no cause for concern. In general, the QQ-plot (the
top right panel) is usually the one in which any lack of model fit become most
clear.
...
Either one, both or none of the parameters in the GPD might be dependent on dose,
...
been suggested by many other authors~\cite{jeffreys, raftery, lindsey, davison}.
...
robust linear model, the MCMC simulated GPD model, and other information
...
Given that ALT I skewed and subject to outliers, it is difficult to justify
either approach.
...
\bibitem{mkada}
H. M'Kada, M. Munteanu, H. Perazzo, Y. Ngo, N. Ramanujam, F. Imbert-Bismut,
V. Ratziu, D. Bonnefont-Rousselot, B. Souberbielle, I. Schuppe-Koistinen,
T. Poynard and the DILI group of the SAFE-T consortium.
What are the best reference values for a normal serum alanine transaminase
activity (ALT)? Impact on the presumed prevalence of drug induced liver
injury (DILI), Regulatory Toxicity and Pharmacology, 60, 290 -- 295, 2011

\bibitem{egp}
I. Papastathopoulos and J. A. Tawn, Extended generalised Pareto models for tail
estimation, Journal of Statistical Planning and Inference, 143, 131 -- 143, 2011
...
\bibitem{sassyreader}
K. S\/orensen, SassyReader -- Open source reader of SAS data sets for Java,
