...
& Heffernan [@southHeff]. For non-clinical trial use, the package also
provides ggplot [@ggplot2] replacement functions for several of the
texmex [@texmex] plot functions.
...
ones) are used, or if binary compressed SAS datasets are used. SAS do not publish the
...
datasets appear to be uncommon.
...
that the treatment decodes need to be added to {\tt alt} we do another left join:
...
The lower left panel of Figure~\ref{fig:thresh} shows the _mean residual life_
...
In Figure~\ref{fig:thresh}, the data are _all_ of the residuals from the robust
